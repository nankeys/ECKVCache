...
std::vector<uint8_t> ErasureCacheClient::get(const std::string& key) {
    ...
    erasures.push_back(-1); // end mark for ISA-L
    ...
    std::vector<std::vector<uint8_t>> recovered_blocks(m, std::vector<uint8_t>(block_size));
    std::vector<uint8_t*> recovery_ptrs(m);
    for (int i = 0; i < m; ++i) recovery_ptrs[i] = recovered_blocks[i].data();

    ec_encode_data(block_size, k, m, g_tbls.data(), block_ptrs.data(), recovery_ptrs.data());
    ...
    for (int i = 0; i < k; ++i) {
        if (block_ptrs[i]) {
            ...
            full_value.insert(full_value.end(), recovery_ptrs[i - k], recovery_ptrs[i - k] + block_size);
        }
    }
